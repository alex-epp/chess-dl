...
namespace chess {
    ...
    public:
        ...
        explicit ZobristHasher(std::array<std::uint64_t, 781> random_numbers = POLYGLOT_RANDOM_NUMBERS);

        Hash operator () (const Board& board);
    private:
        Hash hash_board(const Board& board);
        Hash hash_castling(const Board& board);
        Hash hash_ep_square(const Board& board);
        Hash hash_turn(const Board& board);
